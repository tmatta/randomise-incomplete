...
#--- as y goes up, pr of dropout goes up... slow CT increases pr of dropout  
or_drop <- c(1.5, 2, 2.5, 3, 3.5)
or_drop_labs <- paste0("or_", round(or_drop, 2)*10)
...
mar_mis_output <- miss_output
mar_mod_output <- mod_output
mar_data <- df_list

save(mar_mis_output, file = "4_mar_mis_output.rda")
save(mar_mod_output, file = "4_mar_mod_output.rda")
save(mar_data, file = "4_mar_data.rda")



round(colMeans(miss_output[, 1:3,  1:5]), 2)

round(colMeans(mod_output[, 1, 1:10,  1:5]), 2)
round(colMeans(mod_output[, 2, 1:10,  1:5]), 2)
