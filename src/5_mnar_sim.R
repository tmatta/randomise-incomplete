#==============================================================================#
# Generate MNAR dropout mechanisms for longitudinal data
# T Matta 
#------------------------------------------------------------------------------#
# set.seed(8732)
library(mvtnorm)
...
or_drop <- c(1.5, 2, 2.5, 3, 3.5)
or_drop_labs <- paste0("or_", or_drop*10)

t_labs <- c("t1", "t2", "t3")
par_labs <- c("int", "slp", "grp_slp", "int_se", "slp_se", "grp_slp_se", "sd_int", "sd_slp", "int_slp_cor", "resid")

#=== Simulation Arrays ========================================================#
miss_output <- array(NA, dim = c(reps, length(t_labs), length(or_drop_labs)), 
                    dimnames = list(1:reps, t_labs, or_drop_labs))

mod_output <- array(NA, dim = c(reps, 2, length(par_labs), length(or_drop_labs)), 
                    dimnames = list(1:reps, c("av", "cc"), par_labs, or_drop_labs))
...
for(jj in 1:length(or_drop)){
  ...
  for(rr in 1:reps){
    ...
    # move z-score[i] to z-score[i-1]    
    for (i in 1:nrow(y_l)) y_l$zy_f[i] <- ifelse(y_l$time[i] <= 1, y_l$zy[i + 1], 0) 
           
    y_l$index <- rownames(y_l)

    #--- Final full data data.frame
    y_l <- y_l[, c("index", "id", "time", "x", "x_t", "y", "zy", "zy_f")]
    ...
    d$haz <- 1 / (1 + exp(-(a0 + a1*d_mat$t1 + a2*d_mat$zy_f)))
...
mnar_mis_output <- miss_output
mnar_mod_output <- mod_output
mnar_data <- df_list

save(mnar_mis_output, file = "5_mnar_mis_output.rda")
save(mnar_mod_output, file = "5_mnar_mod_output.rda")
save(mnar_data, file = "5_mnar_data.rda")
